...
export const Sidebar = () => {
    ...
    const border = "border-b border-gray-8"

    return (
        <div className="w-[306px] flex justify-center flex-col border border-gray-8 bg-gray-2 rounded-[20px] h-fit">
            <ListLayout title="Category" border={border}>
                {categories.map((el) => <ListItem text={el} key={el} />)}
            </ListLayout>
            <ListLayout title="Weight" border={border}>
                {weights.map((el) => <ListItem text={el} key={el} />)}
            </ListLayout>
            <ListLayout title="Color" border={border}>
                <ul className="flex  flex-wrap gap-[13px] w-[264px]">
                    {colors.map((el) => <li className={`w-[22px] h-[22px] ${el} rounded-[50%]`} key={el}></li>)}
                </ul>
            </ListLayout>
            <ListLayout title="Price" border={border}>
                <div className="flex justify-center items-center flex-col gap-[19px]">
                    <span className="font-serif font-normal text-base leading-6 tracking-0.32 text-black px-[84px] py-[12px] border border-gray-8 bg-white rounded-[10px]">$130 - $250</span>
                    <input type="range" min={25} max={1000} className="custom-range" />
                </div>
            </ListLayout>
            <ListLayout title="Tags">
                <ul className="grid grid-cols-3 gap-[10px]">
                    {tags.map((el) => <li className="font-serif font-light text-[13px] flex justify-center items-center leading-7 tracking-0.48 text-gray-7 border border-gray-8 rounded-[10px] px-[10px] py-[2px]" key={el}>{el}</li>)}
                </ul>
            </ListLayout>
        </div>
    )
